...
-- deflitmus IRIW_rmw := RMW x // 0 || R x // 1 ; R y // 0 || R y // 1; R x // 0 || RMW y // 0
-- expect 𐄂 -- tso is multicopy atomic anyway...

-- deflitmus two_rmws := RMW x // 0; R x // 1 || RMW x // 0; R x // 1
-- expect 𐄂 -- this actually should be allowed, because it will be the result if the write fails!

-- deflitmus dekkers_rmw_fence_unsuccessful := W x = 1; RMW y // 0; R z //0 || W z = 1; RMW y // 0; R x // 0  --
-- expect ✓

-- deflitmus dekkers_rmw_fence_successful := W x = 1; RMW y // 0; R z //0 || W z = 1; RMW y // 1; R x // 0  --
-- expect 𐄂 even without the additional PPO
...
deflitmus r_mfences := W x = 1; Fence; W y = 1 || W y = 2; Fence; R x // 0; R y // 2
 expect ✓ -- huh, really?
