...
deflitmus MP_sys := W x=1; W y=1 || R y // 1; R x // 0 expect ✓

deflitmus MP_sys_F := W x=1; Fence; W y=1 || R y //1; Fence; R x // 0  expect 𐄂

deflitmus MP_cta_F := W x=1; Fence.cta_sc; W y=1 || R y // 1; Fence.cta_sc; R x // 0
 where sys := { {T0}, {T1} } expect ✓

deflitmus SB_sys := W x=1; R y //0 || W y=1; R x // 0  expect ✓

deflitmus SB_sys_F := W x=1; Fence; R y //0 || W y=1; Fence; R x // 0  expect 𐄂

deflitmus IRIW_sys := W x=1 || R x // 1 ; R y // 0 || R y // 1; R x // 0 || W y=1  expect ✓

deflitmus IRIW_sys_F := W x=1 || R x // 1 ; Fence; R y // 0 || R y // 1; Fence; R x // 0 || W y=1  expect 𐄂

deflitmus LB_sys := R x // 1; W y = 1 || R y // 1; W x=1  expect 𐄂
