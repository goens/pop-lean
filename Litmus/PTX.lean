...
deflitmus IRIW_2ctas_all_cta_scopes :=  W.cta_rlx x=1 ||  R.cta_rlx x // 1 ; Fence.cta_sc;  R.cta_rlx y // 0 || R.cta_rlx y // 1; Fence.cta_sc; R.cta_rlx x // 0 || W.cta_rlx y=1
  where sys := { {T0, T1}, {T2, T3} }
 expect ✓
...
deflitmus MP_fence_cta_1fence := W x=1; Fence.cta_sc; W y=1 || R y // 1; Fence.cta_sc; R x // 0
 where sys := { {T0}, {T1} }
 expect ✓
