...
def interactiveExecutionSingle : Litmus.Test → List  Nat → IO.FS.Stream → IO (Except String SearchState)
  | .mk initTrans initProgSt expected _descr initSysSt name axiomatic guideTraces, partialTraceInit, stdin => do
    ...
    println! "=======  SUMMARY  ======="
    println! "========================="
    println! s!"hint for {name} := {partialTrace}\n" ++
...
def selectLitmusLoop : List Litmus.Test → IO.FS.Stream → IO (Except String Litmus.Test)
  | tests, stdin => do
    let litmusHintColors := tests.map λ test =>
      let guideTrace := match test.guideTraces.head? with
        | none => []
        | some trace => trace
      let resHint := test.initState.applyTrace (test.initTransitions ++ guideTrace)
      match resHint with
